...
void ShowSimulation()
{
  ...
  static float simulation_window_size = 500.0f;
  static float Ball_X{}, Ball_Y{}, Box_X{}, Box_Y{};
  if (ImGui::TreeNodeEx("Simulation Information")) {
    ImGui::Text("Ball is at: [%f, %f]", Ball_X, Ball_Y);
    ImGui::Text("Box is at: [%f, %f]", Box_X, Box_Y);

    ImGui::SliderFloat("Simulation Window size", &simulation_window_size, 20, 1000, "%.0f");
    ImGui::TreePop();
  }
