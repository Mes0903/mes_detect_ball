...
int main([[maybe_unused]] int argc, char **argv)
{
  ...
  if (case_num == 1)
  {
    ...
    Eigen::MatrixXd train_X = Load_Matrix::readDataSet(filepath + "/include/dataset/ball_train_xn_2.txt", 50996, 5); // file, row, col

    puts("reading training label...");
    Eigen::VectorXd train_Y = Load_Matrix::readLabel(filepath + "/include/dataset/ball_train_yn_2.txt", 50996); // file, segment num(row)

    puts("reading testing data...");
    Eigen::MatrixXd test_X = Load_Matrix::readDataSet(filepath + "/include/dataset/ball_test_xn_2.txt", 21857, 5);

    puts("reading testing label...");
    Eigen::VectorXd test_Y = Load_Matrix::readLabel(filepath + "/include/dataset/ball_test_yn_2.txt", 21857);
