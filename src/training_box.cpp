...
int main([[maybe_unused]] int argc, char **argv)
{
  ...
  if (case_num == 1) {
    ...
    Eigen::MatrixXd train_X = LoadMatrix::readDataSet(filepath + "/include/dataset/box_train_x.txt", 50996, 10);    // file, row, col

    puts("reading training label...");
    Eigen::VectorXd train_Y = LoadMatrix::readLabel(filepath + "/include/dataset/box_train_y.txt", 50996);    // file, segment num(row)

    puts("reading testing data...");
    Eigen::MatrixXd test_X = LoadMatrix::readDataSet(filepath + "/include/dataset/box_test_x.txt", 21857, 10);

    puts("reading testing label...");
    Eigen::VectorXd test_Y = LoadMatrix::readLabel(filepath + "/include/dataset/box_test_y.txt", 21857);
    ...
    for (int i = 0; i < sample; ++i) {
      ...
      FileHandler::store_weight(confusion_matrix, filepath + "/include/weight_data/adaboost_box_weight.txt", A, normalizer);
    }
  }
  else {
    puts("reading testing data...");
    ...
    puts("Load Weighting...");
    FileHandler::load_weight(filepath + "/include/weight_data/adaboost_box_weight.txt", A, normalizer);
