...
weak_learner::fit(const Eigen::MatrixXd &train_X, const Eigen::VectorXd &train_Y, const Eigen::MatrixXd &train_weight, uint32_t Iterations)
{
 ...
 */
Eigen::ArrayXd weak_learner::logistic(const Eigen::ArrayXd &x)
{
  return ((x / 2).tanh() + 1) / 2;
}
...
{
  int N = 5;    // The number of weighting
